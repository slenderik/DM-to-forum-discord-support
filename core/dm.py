...
import disnake
from disnake.ext.commands import MissingRequiredArgument, CommandError
...
class Chat:
    """Represents a disnake Modmail thread"""
    ...
    @property
    def channel(self) -> typing.Union[disnake.TextChannel, disnake.DMChannel]:
        return self._channel

    @property
    def recipient(self) -> typing.Optional[typing.Union[disnake.User, disnake.Member]]:
        return self._recipient

    @property
    def recipients(self) -> typing.List[typing.Union[disnake.User, disnake.Member]]:
    ...
    async def get_genesis_message(self) -> disnake.Message:
    ...
    async def setup(self, *, creator=None, category=None, initial_message=None):
        ...
        overwrites = {self.bot.modmail_guild.default_role: disnake.PermissionOverwrite(read_messages=False)}
        ...
        except disnake.HTTPException as e:  # Failed to create due to missing perms.
            logger.critical("An error occurred while creating a thread.", exc_info=True)
            self.manager.cache.pop(self.id)

            embed = disnake.Embed(color=self.bot.error_color)
        ...
        async def send_persistent_notes():
            ...
            for note in notes:
                ...
                data = {
                    "id": round(time.time() * 1000 - disnake.utils.disnake_EPOCH) << 22,
                    "attachments": {},
                    "embeds": {},
                ...
                message = disnake.Message(state=State(), channel=self.channel, data=data)
    ...
    def _format_info_embed(self, user, log_url, log_count, color):
        ...
        if self.bot.config["thread_show_account_age"]:
            created = disnake.utils.format_dt(user.created_at, "R")
            user_info.append(f" was created {created}")

        embed = disnake.Embed(color=color, description=user.mention, timestamp=time)
        ...
        if member is not None:
            ...
            if self.bot.config["thread_show_join_age"]:
                joined = disnake.utils.format_dt(member.joined_at, "R")
    ...
    async def close(
        self,
        *,
        closer: typing.Union[disnake.Member, disnake.User],
        after: int = 0,
        silent: bool = False,
    ...
    async def _close(self, closer, silent=False, delete_channel=True, message=None, scheduled=False):
        ...
        if self.channel:
            log_data = await self.bot.api.post_log(
                self.channel.id,
                {
                    ...
                    "closed_at": str(disnake.utils.utcnow()),
        ...
        embed = disnake.Embed(description=desc, color=self.bot.error_color)
        ...
        if self.bot.log_channel is not None and self.channel is not None:
            if self.bot.config["show_log_url_button"]:
                ...
                view.add_item(disnake.ui.Button(label="Log link", url=log_url, style=disnake.ButtonStyle.url))
            else:
                view = None
    ...
    async def find_linked_messages(
        ...
        message1: disnake.Message = None,
        note: bool = True,
    ) -> typing.Tuple[disnake.Message, typing.List[typing.Optional[disnake.Message]]]:
    ...
    async def delete_message(
        self, message: typing.Union[int, disnake.Message] = None, note: bool = True
    ) -> None:
    ...
    async def edit_dm_message(self, message: disnake.Message, content: str) -> None:
        try:
        ...
        for msg in linked_messages:
            embed = msg.embeds[0]
            if isinstance(msg.channel, disnake.TextChannel):
    ...
    async def note(
        self, message: disnake.Message, persistent=False, thread_creation=False
    ) -> disnake.Message:
    ...
    async def reply(
        self, message: disnake.Message, anonymous: bool = False, plain: bool = False
    ) -> typing.Tuple[typing.List[disnake.Message], disnake.Message]:
    ...
    async def send(
        ...
        system_avatar_url = "https://disnakeapp.com/assets/f78426a064bc9dd24847519259bc42af.png"

        if not note:
            if anonymous and from_mod and not isinstance(destination, disnake.TextChannel):
                ...
                embed.set_author(
                    name=name,
                    icon_url=avatar_url,
                    url=f"https://disnakeapp.com/channels/{self.bot.guild.id}#{message.id}",
                )
            else:
                # Normal message
                name = str(author)
                ...
                embed.set_author(
                    name=name,
                    icon_url=avatar_url,
                    url=f"https://disnakeapp.com/users/{author.id}#{message.id}",
                )
        else:
        ...
        for i in message.stickers:
            if i.format in (disnake.StickerFormatType.png, disnake.StickerFormatType.apng):
                images.append((i.url, i.name, True))
            elif i.format == disnake.StickerFormatType.lottie:
        ...
        for url, filename, is_sticker in images:
            if (
                ...
                img_embed = disnake.Embed(color=color)
        ...
        if from_mod:
            ...
            if anonymous and isinstance(destination, disnake.TextChannel):
        ...
        if plain:
            if from_mod and not isinstance(destination, disnake.TextChannel):
                # Plain to user
    ...
    async def add_users(self, users: typing.List[typing.Union[disnake.Member, disnake.User]]) -> None:
        topic = ""
    ...
    async def remove_users(self, users: typing.List[typing.Union[disnake.Member, disnake.User]]) -> None:
        topic = ""
...
class ChatManager:
    ...
    async def create(
        self,
        recipient: typing.Union[disnake.Member, disnake.User],
        *,
